...
int main()
{
    ...
    initgraph(&graphicdriver,&graphicmode,NULL);
...
void PaintWorld(char world[][MAX_COLUMNS],int iteration){
    ...
    outtextxy(150,330,text);
    outtextxy(420,330,integer);
    setcolor(BLUE);
    for (int row=0,world_x=0;row<315;row+=15,world_x++) {
        for (int column=0,world_y=0; column < 660; column+=15,world_y++) {
            if (world[world_x][world_y] == 'x')
                ...
                setcolor(WHITE);
            bar(0 + column, 0+row, 15 + column, 15+row);
        }
    }
