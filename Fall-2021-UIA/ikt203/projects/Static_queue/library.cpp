...
int successor(Queue *queue, int value){
    value++;
    if ((value % queue->capacity-INDEX_CORRECT) == 0){
        value = 0;
    }
    return value;
}
...
void enqueue(Queue *queue, int x, int y){
    if (is_full(queue))
        ...
        int index = successor(queue, queue->rear);
        queue->array[index].x = x;
        queue->array[index].y = y;
        queue->size++;
    }
}
...
Position *dequeue(Queue *queue){
    ...
    if (is_empty(queue)){
        ...
        Position *old_position = (Position *)malloc(sizeof(Position));
