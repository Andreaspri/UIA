...
void PaintWorld(char world[][MAX_COLUMNS],int iteration);
...
int main()
{
    ...
    initgraph(&graphicdriver,&graphicmode,NULL,MAX_COLUMNS*PIXEL,MAX_ROWS*PIXEL+100);
...
void PaintWorld(char world[][MAX_COLUMNS],int iteration){
    ...
    char text[] = {"Iteration:"};
    settextstyle(8,0,5);
    sprintf(integer, "%d", iteration);
    outtextxy(150,MAX_ROWS*PIXEL+15,text);
    outtextxy(420,MAX_ROWS*PIXEL+15,integer);
    setcolor(BLUE);
    for (int row=0,world_x=0;row<PIXEL*MAX_ROWS;row+=PIXEL,world_x++) {
        for (int column=0,world_y=0; column < PIXEL*MAX_COLUMNS-PIXEL; column+=PIXEL,world_y++) {
            if (world[world_x][world_y] == 'x')
                setcolor(GREEN);
            else if (world[world_x][world_y] == '!' || world[world_x][world_y] == '-')
                setcolor(BLUE);
            else
                setcolor(WHITE);
            bar(0 + column, 0+row, PIXEL + column, PIXEL+row);
        }
    }
...
void NextGeneration(char oldWorld[][MAX_COLUMNS], char newWorld[][MAX_COLUMNS])
{
    ...
    int alive_cells = 0;
    for (int row = 1; row < MAX_ROWS-1;row++){
        for (int column = 1; column < MAX_COLUMNS-2;column++){
